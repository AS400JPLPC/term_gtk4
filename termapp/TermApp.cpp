...
int main(int argc, char *argv[])
{
    ...
    char *new_path = g_strdup_printf("PATH=%s:%s", env_path, Env_File(argv[1]));
...
/// contrôle autorisation traitement --> protection
    ...
    if (argc == 2 )  {
        ...
        if ( false == isDir_File((char*)argv[1]) )         return EXIT_FAILURE;     // contrôle is directory
        if ( false == exists_File((char*)argv[1]) )        return EXIT_FAILURE;     // contrôle si programme
        gchar *arg_2[] = { Pgm_File(argv[1]), NULL};
        command = arg_2;
    }
    ...
    if (argc == 3 )  {
        ...
        char * arg_3[] = { Pgm_File(argv[1]),(gchar*)"-c", (gchar*) argv[3], NULL};	    /// arg[2] P_pgm , [2] Pid_msgq
        command = arg_3;
    }
